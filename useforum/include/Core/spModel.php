...
class spPager {
	...
	private function runpager($func_name, $func_args){
		...
		if($total_count > $pageSize){
			...
			if($total_page > 10){ //页数大于10，开始省略
				$this->pageData['all_pages'] = array_slice($this->pageData['all_pages'],($page - 2) < 1 ? 0 : ($page - 3),6);
			//分页增加行
			}
...
class spVerifier {
	...
	private function email($val, $right){
		return $right == ( preg_match('/^[A-Za-z0-9]+([._\-\+]*[A-Za-z0-9]+)*@([A-Za-z0-9-]+\.)+[A-Za-z]{2,4}+$/', $val) != 0 );
	}
	/** 
